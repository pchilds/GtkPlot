				...
				if (((plot->flagd)&GTK_PLOT_POLAR_DISP_PNT)==0) /* straight lines */
				{
					for (k=0; k<(plt->ind->len); k++)
					{
						...
						for (ssx=MY_2PI; ssx>-10; ssx-=MY_2PI)
						{
							...
							{
								{rn=g_array_index((plot->rdata), gdouble, j); tn=ssx+g_array_index((plot->thdata), gdouble, j+(plot->rdata->len));}
...
static void gtk_plot_polar_class_init(GtkPlotPolarClass *klass)
{
	...
	g_object_class_install_property(obj_klass, PROP_HV, g_param_spec_boolean("transpose", "alternate display", "Display from second set in thdata", FALSE, G_PARAM_READWRITE));
