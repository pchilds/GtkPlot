...
#define GTK_PLOT_LOG_LOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_PLOT_TYPE_LOG_LOG, GtkPlotLogLogPrivate))
...
typedef enum {
  GTK_PLOT_LOG_LOG_BORDERS_LT = 1 << 0,
  GTK_PLOT_LOG_LOG_BORDERS_RT = 1 << 1,
  GTK_PLOT_LOG_LOG_BORDERS_DN = 1 << 2,
  GTK_PLOT_LOG_LOG_BORDERS_UP = 1 << 3
} GtkPlotLogLogBorders;
typedef enum {
  GTK_PLOT_LOG_LOG_AXES_LW = 1 << 0,
  GTK_PLOT_LOG_LOG_AXES_RW = 1 << 1,
  GTK_PLOT_LOG_LOG_AXES_DW = 1 << 2,
  GTK_PLOT_LOG_LOG_AXES_UW = 1 << 3,
  GTK_PLOT_LOG_LOG_AXES_LR = 1 << 4,
  GTK_PLOT_LOG_LOG_AXES_LT = 1 << 5
} GtkPlotLogLogAxes;
G_DEFINE_TYPE (GtkPlotLogLog, gtk_plot_log_log, GTK_TYPE_PLOT);
enum {PROP_0, PROP_BXN, PROP_BXX, PROP_BYN, PROP_BYX, PROP_XTN, PROP_YTN, PROP_FA};
enum {MOVED, KEY_CHANGED, LAST_SIGNAL};
static guint gtk_plot_log_log_signals[LAST_SIGNAL]={0};
typedef struct _GtkPlotLogLogPrivate GtkPlotLogLogPrivate;
struct xs {gdouble xmin, ymin, xmax, ymax;};
struct tk {guint xj, yj, xn, yn;};
struct _GtkPlotLogLogPrivate {struct xs bounds, rescale; struct tk ticks, range; guint flaga, flagr;};
...
static void drawz(GtkWidget *widget, cairo_t *cr) {
  ...
  if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_DRG)!=0) {
    ...
    if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_OUT)==0) {
  ...
  cairo_stroke(cr);
  if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_SGL)==0) {
    dt=1;
    cairo_set_dash(cr, &dt, 1, 0);
    if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_VRT)!=0) {
    ...
    if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_HZT)!=0) {
...
static void drawk(GtkWidget *widget, cairo_t *cr) {
  ...
  do {
    ...
    if (plot->kdata && k<(plot->kdata->len)) {
      ...
      if (fg&GTK_PLOT_LOG_LOG_KEY_CLR) yv=0.0;
      switch ((fg&GTK_PLOT_LOG_LOG_KEY_DSH)>>1) {
        case 2:
...
static void draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gchar *str1=NULL;
  gchar lbl[BFL];
  gdouble av, dt, vv, wv, zv;
  gint dtt, ft, hg, j, k, lt, st, tn, tnn, tx, wd, xa, xl, xr, xr2, xt, xu, xv, xvn, xw, ya, yl, yr, yr2, yu, yv, yvn, yw;
  GtkPlot *plt;
  GtkPlotLogLog *plot;
  ...
  priv=GTK_PLOT_LOG_LOG_GET_PRIVATE(plot);
  priv->flaga&=GTK_PLOT_LOG_LOG_AXES_LT|GTK_PLOT_LOG_LOG_AXES_LR;
  lyt=pango_cairo_create_layout(cr);
  pango_layout_set_font_description(lyt, plt->lfont);
  ...
  priv->flaga|=GTK_PLOT_LOG_LOG_AXES_UW;
  yl=ya-yw*WGP;
  g_object_unref(lyt);
  lyt=pango_cairo_create_layout(cr);
  pango_layout_set_font_description(lyt, plt->afont);
  xu=xw-xr;
  xa=dtt;
  priv->flaga|=GTK_PLOT_LOG_LOG_AXES_RW;
  xl=dtt+xw*WGP;
  g_object_unref(lyt);
  priv->range.xj=xl;
  priv->range.yj=yu;
...
/* draw y wiggles */
  ...
  cairo_curve_to(cr, xa-WHP*yw, ya-WMP*yw, xa+WHP*yw, ya-WMP*yw, xa, yl);
  yr-=2;
...
/* draw minor x grid bits */
  ...
  pango_layout_get_pixel_size(lyt, &wd, &hg);
  if (((priv->flaga)&GTK_PLOT_LOG_LOG_AXES_LT)!=0) {
  ...
  pango_layout_get_pixel_size(lyt, &wd, &hg);
  if (((priv->flaga)&GTK_PLOT_LOG_LOG_AXES_LR)!=0) {
    ...
    for (j=ceil(priv->bounds.ymin); j<floor(priv->bounds.ymax); j++) {
      tn=yl-(j-priv->bounds.ymin)*(yl-yu)/(priv->bounds.ymax-priv->bounds.ymin);
      cairo_save(cr);
      cairo_set_line_width(cr, 1);
      cairo_set_dash(cr, &dt, 1, 0);
  ...
  if ((plot->xdata)&&(plot->ydata)) { /* plot data */
    if (((plot->flagd)&GTK_PLOT_LOG_LOG_DISP_LIN)!=0) {
      cairo_set_line_width(cr, (plot->linew));
      if (((plot->flagd)&GTK_PLOT_LOG_LOG_DISP_PTS)!=0) { /* lines and points */
        for (k=0; k<plt->ind->len; k++) {
          cairo_set_dash(cr, NULL, 0, 0);
          if (plot->kdata && k<(plot->kdata->len)) {
            fg=g_array_index(plot->kdata, guint, k);
            if (fg&GTK_PLOT_LOG_LOG_KEY_CLR) continue;
            switch ((fg&GTK_PLOT_LOG_LOG_KEY_DSH)>>1) {
              case 2:
                cairo_set_dash(cr, dsh2, sizeof(dsh2)/sizeof(dsh2[0]), 0);
                break;
          ...
          if (g_array_index(plot->ydata, gdouble, ft)<DZE) yv=yl+1.0;
          else yv=yl+(yu-yl)*(log10(g_array_index(plot->ydata, gdouble, ft))-priv->bounds.ymin)/(priv->bounds.ymax-priv->bounds.ymin);
          if (xv<xl) {
            if (yv>yl) xt=GTK_PLOT_LOG_LOG_BORDERS_LT|GTK_PLOT_LOG_LOG_BORDERS_DN;
            else if (yv<yu) xt=GTK_PLOT_LOG_LOG_BORDERS_LT|GTK_PLOT_LOG_LOG_BORDERS_UP;
            else xt=GTK_PLOT_LOG_LOG_BORDERS_LT;
          } else if (xv>xu) {
            if (yv>yl) xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_DN;
            else if (yv<yu) xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_DN;
            else xt=GTK_PLOT_LOG_LOG_BORDERS_RT;
          } else if (yv>yl) xt=GTK_PLOT_LOG_LOG_BORDERS_DN;
          else if (yv<yu) xt=GTK_PLOT_LOG_LOG_BORDERS_UP;
          else {
            xt=0;
          ...
          for (j=st+ft; j<lt; j+=st) {
            ...
            if (g_array_index(plot->ydata, gdouble, j)<DZE) yvn=yl+1.0; 
            else yvn=yl+(yu-yl)*(log10(g_array_index(plot->ydata, gdouble, j))-priv->bounds.ymin)/(priv->bounds.ymax-priv->bounds.ymin);
            if (xvn<xl) {
              if (yvn>yl) {
                if (xt==0) {
                ...
                xt=GTK_PLOT_LOG_LOG_BORDERS_LT|GTK_PLOT_LOG_LOG_BORDERS_DN;
              } else if (yvn<yu) {
                if (xt==0) {
                  tx=yvn+(yvn-yv)*(xl-xvn)/(xvn-xv);
                ...
                xt=GTK_PLOT_LOG_LOG_BORDERS_LT|GTK_PLOT_LOG_LOG_BORDERS_UP;
              } else if (xt==0) {
                cairo_line_to(cr, xl, yvn+(yvn-yv)*(xl-xvn)/(xvn-xv));
                cairo_stroke(cr);
                xt=GTK_PLOT_LOG_LOG_BORDERS_LT;
              } else if (xt==GTK_PLOT_LOG_LOG_BORDERS_DN) {
                tx=yvn+(yvn-yv)*(xl-xvn)/(xvn-xv);
                if (tx>yl) {
                ...
                xt=GTK_PLOT_LOG_LOG_BORDERS_LT;
              } else if (xt==GTK_PLOT_LOG_LOG_BORDERS_UP) {
                tx=yvn+(yvn-yv)*(xl-xvn)/(xvn-xv);
                if (tx<yu) {
                  cairo_move_to(cr, xl, tx);
              ...
              if (yvn>yl) {
                ...
                xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_DN;
              } else if (yvn<yu) {
                if (xt==0) {
                  tx=yv+(yvn-yv)*(xu-xv)/(xvn-xv);
                ...
                xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_UP;
              }
              if (xt==0) {
                cairo_line_to(cr, xu, yv+(yvn-yv)*(xu-xv)/(xvn-xv));
                cairo_stroke(cr);
                xt=GTK_PLOT_LOG_LOG_BORDERS_RT;
              } else if (xt==GTK_PLOT_LOG_LOG_BORDERS_DN) {
                tx=yv+(yvn-yv)*(xu-xv)/(xvn-xv);
                if (tx>yl) {
              ...
              if (xt==0) {
                ...
                cairo_stroke(cr);
              } else if (xt==GTK_PLOT_LOG_LOG_BORDERS_LT) {
              ...
              } else if (xt==GTK_PLOT_LOG_LOG_BORDERS_RT) {
              ...
              xt=GTK_PLOT_LOG_LOG_BORDERS_DN;
            } else if (yvn<yu) {
              if (xt==0) {
              ...
              xt=GTK_PLOT_LOG_LOG_BORDERS_UP;
            } else { /* within range */
              if ((xt&GTK_PLOT_LOG_LOG_BORDERS_LT)!=0) {
                if ((xt&GTK_PLOT_LOG_LOG_BORDERS_DN)!=0) {
                  tx=yv+(yvn-yv)*(xl-xv)/(xvn-xv);
                  if (tx>yl) cairo_move_to(cr, xv+(xvn-xv)*(yl-yv)/(yvn-yv), yl);
                  else cairo_move_to(cr, xl, tx);
                } else if ((xt&GTK_PLOT_LOG_LOG_BORDERS_UP)!=0) {
                  tx=yv+(yvn-yv)*(xl-xv)/(xvn-xv);
                  if (tx<yu) cairo_move_to(cr, xv+(xvn-xv)*(yu-yv)/(yvn-yv), yu);
                  else cairo_move_to(cr, xl, tx);
                } else cairo_move_to(cr, xl, yv+(yvn-yv)*(xl-xv)/(xvn-xv));
              } else if ((xt&GTK_PLOT_LOG_LOG_BORDERS_RT)!=0) {
                if ((xt&GTK_PLOT_LOG_LOG_BORDERS_DN)!=0) {
                  tx=yvn+(yvn-yv)*(xu-xvn)/(xvn-xv);
                  if (tx>yl) cairo_move_to(cr, xvn+(xvn-xv)*(yl-yvn)/(yvn-yv), yl);
                  else cairo_move_to(cr, xl, tx);
                } else if ((xt&GTK_PLOT_LOG_LOG_BORDERS_DN)!=0) {
                  tx=yvn+(yvn-yv)*(xu-xvn)/(xvn-xv);
                  if (tx<yu) cairo_move_to(cr, xvn+(xvn-xv)*(yu-yvn)/(yvn-yv), yu);
                  else cairo_move_to(cr, xl, tx);
                } else cairo_move_to(cr, xu, yvn+(yvn-yv)*(xu-xvn)/(xvn-xv));
              } else if ((xt&GTK_PLOT_LOG_LOG_BORDERS_DN)!=0) cairo_move_to(cr, xv+(xvn-xv)*(yl-yv)/(yvn-yv), yl);
              else if ((xt&GTK_PLOT_LOG_LOG_BORDERS_UP)!=0) cairo_move_to(cr, xv+(xvn-xv)*(yu-yv)/(yvn-yv), yu);
        ...
        for (k=0;k<(plt->ind->len);k++) {
          ...
          if (xv<xl) {
            ...
            if (yv>yl) xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_DN;
            else if (yv<yu) xt=GTK_PLOT_LOG_LOG_BORDERS_RT|GTK_PLOT_LOG_LOG_BORDERS_UP;
            else xt=GTK_PLOT_LOG_LOG_BORDERS_RT;
          } else if (yv>yl)xt=GTK_PLOT_LOG_LOG_BORDERS_DN;
          else if (yv<yu) xt=GTK_PLOT_LOG_LOG_BORDERS_UP;
          else {
    ...
    } else if (((plot->flagd)&GTK_PLOT_LOG_LOG_DISP_PTS)!=0) { /* points only */
      for (k=0;k<(plt->ind->len);k++) {
        if (plot->kdata && k<(plot->kdata->len) && (g_array_index(plot->kdata, guint, k)&GTK_PLOT_LOG_LOG_KEY_CLR)) continue;
        dtt=fmod(k,plt->rd->len);
        vv=g_array_index(plt->rd, gdouble, dtt);
        wv=g_array_index(plt->gr, gdouble, dtt);
        zv=g_array_index(plt->bl, gdouble, dtt);
        ...
        if (lt>(plot->ydata->len)) lt=plot->ydata->len;
        for (j=ft; j<lt; j+=st) {
          if ((g_array_index(plot->xdata, gdouble, j)<DZE)||(g_array_index(plot->ydata, gdouble, j)<DZE)) continue;
          xv=xl+(xu-xl)*(log10(g_array_index(plot->xdata, gdouble, j))-priv->bounds.xmin)/(priv->bounds.xmax-priv->bounds.xmin);
          yv=yl+(yu-yl)*(log10(g_array_index(plot->ydata, gdouble, j))-priv->bounds.ymin)/(priv->bounds.ymax-priv->bounds.ymin);
...
static void gtk_plot_log_log_redraw(GtkWidget *widget) {
  GdkRegion *region;
  GdkWindow *wdw;
...
gboolean gtk_plot_log_log_update_scale_pretty(GtkWidget *widget, gdouble xl, gdouble xu, gdouble yl, gdouble yu) {
  ...
  ut=(gint) ceil(log10(yx));
  if (yn<=DZE) {
    lt=ut-6;
    priv->ticks.yn=4;
  } else {
    lt=(gint) floor(log10(yn));
    tk=ut-lt;
    if (tk<=0) return TRUE;
    else if (tk>6) {
      priv->ticks.yn=4;
      lt=ut-6;
    } else if (tk>4) priv->ticks.yn=4;
    else priv->ticks.yn=8;
  }
  priv->bounds.ymin=(gdouble) lt;
  priv->bounds.ymax=(gdouble) ut;
  gtk_plot_log_log_redraw(widget);
  return FALSE;
}
...
gboolean gtk_plot_log_log_print_eps(GtkWidget *plot, gchar* fout) {
  cairo_t *cr;
...
gboolean gtk_plot_log_log_print_png(GtkWidget *plot, gchar* fout) {
  cairo_t *cr;
  cairo_surface_t *surface;
...
gboolean gtk_plot_log_log_print_svg(GtkWidget *plot, gchar* fout) {
  cairo_t *cr;
  cairo_surface_t *surface;
...
static gboolean gtk_plot_log_log_button_press(GtkWidget *widget, GdkEventButton *event) {
  gdouble s;
  gint d, hg, j, wd, xw;
  GtkPlot *plt;
  GtkPlotLogLog *plot;
  ...
  if ((priv->flagr)==0) {
    ...
    if (((plot->zmode)&(GTK_PLOT_LOG_LOG_ZOOM_SGL|GTK_PLOT_LOG_LOG_ZOOM_HZT))!=0) {
    ...
    if (((plot->zmode)&(GTK_PLOT_LOG_LOG_ZOOM_SGL|GTK_PLOT_LOG_LOG_ZOOM_VRT))!=0) {
...
static gboolean gtk_plot_log_log_motion_notify(GtkWidget *widget, GdkEventMotion *event) {
  gdouble dx, dy;
  GtkPlotLogLog *plot;
  GtkPlotLogLogPrivate *priv;
  ...
  if ((dx>=0)&&(dy>=0)&&(dx<=1)&&(dy<=1)) {
    ...
    plot->yps=exp(G_LN10*(priv->bounds.ymax*dy+priv->bounds.ymin*(1-dy)));
    g_signal_emit(plot, gtk_plot_log_log_signals[MOVED], 0);
  }
  return FALSE;
}

static gboolean gtk_plot_log_log_button_release(GtkWidget *widget, GdkEventButton *event) {
  ...
  switch (priv->flagr) {
    case 1:
      if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_SGL)==0) {
        ...
        if (((xn>DZE)||(xn<NZE))&&((yn>DZE)||(yn<NZE))) {
          if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_DRG)!=0) gtk_plot_log_log_update_scale_pretty(widget, exp(G_LN10*(priv->bounds.xmin-xn)), exp(G_LN10*(priv->bounds.xmax-xn)), exp(G_LN10*(priv->bounds.ymin-yn)), exp(G_LN10*(priv->bounds.ymax-yn)));
          else if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_OUT)==0) gtk_plot_log_log_update_scale_pretty(widget, exp(G_LN10*priv->rescale.xmin), exp(G_LN10*priv->rescale.xmax), exp(G_LN10*priv->rescale.ymin), exp(G_LN10*priv->rescale.ymax));
          else {
            ...
            if (s>0) {
              ...
              if (s>0) {
                ...
                yx+=priv->bounds.ymax;
                gtk_plot_log_log_update_scale_pretty(widget, exp(G_LN10*xn), exp(G_LN10*xx), exp(G_LN10*yn), exp(G_LN10*yx));
              } else if (s<0) {
      ...
      } else if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_OUT)==0) {
        ...
        gtk_plot_log_log_update_scale_pretty(widget, exp(G_LN10*xn), exp(G_LN10*xx), yn, yx);
      } else {
    ...
    case 2:
      if (((event->y)<=2*SZ[j]+1.0)&&((event->x)>=xw-2*SZ[j]-1.0)) {
        if (((plot->zmode)&(GTK_PLOT_LOG_LOG_ZOOM_DRG|GTK_PLOT_LOG_LOG_ZOOM_OUT))==0) {
          if (((plot->zmode)&(GTK_PLOT_LOG_LOG_ZOOM_SGL))!=0) plot->zmode>>=1;
          plot->zmode|=GTK_PLOT_LOG_LOG_ZOOM_DRG;
        } else (plot->zmode)--;
    ...
    case 3:
      if (((event->y)<=2*SZ[j]+1.0)&&((event->x)>=xw-4*SZ[j]-2.0)&&((event->x)<=xw-2*SZ[j]-1.0)) {
        plot->zmode-=GTK_PLOT_LOG_LOG_ZOOM_HZT;
        if (((plot->zmode)&(GTK_PLOT_LOG_LOG_ZOOM_SGL|GTK_PLOT_LOG_LOG_ZOOM_HZT|GTK_PLOT_LOG_LOG_ZOOM_VRT))==0) {
          if (((plot->zmode)&GTK_PLOT_LOG_LOG_ZOOM_DRG)!=0) plot->zmode|=GTK_PLOT_LOG_LOG_ZOOM_VRT|GTK_PLOT_LOG_LOG_ZOOM_HZT;
          else plot->zmode|=GTK_PLOT_LOG_LOG_ZOOM_SGL;
        }
        ...
        do {
	      ...
	      if (s>(event->y)) {
	        if (d+4==priv->flagr) {
	          ...
	          (*ptr)^=GTK_PLOT_LOG_LOG_KEY_CLR;
              g_signal_emit(plot, gtk_plot_log_log_signals[KEY_CHANGED], 0);
              gtk_plot_log_log_redraw(widget);
	        }
	        break;
	      }
...
static void gtk_plot_log_log_finalise(GtkPlotLogLog *plot) {
  if (plot->xlab) {
    g_free(plot->xlab);
    plot->xlab=NULL;
  }
...
static void gtk_plot_log_log_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
  switch (prop_id) {
    case PROP_BXN:
      if (g_value_get_double(value)>DZE) GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.xmin=log10(g_value_get_double(value));
      break;
    case PROP_BXX:
      if (g_value_get_double(value)>DZE) GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.xmax=log10(g_value_get_double(value));
      break;
    case PROP_BYN:
      if (g_value_get_double(value)>DZE) GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.ymin=log10(g_value_get_double(value));
      break;
    case PROP_BYX:
      if (g_value_get_double(value)>DZE) GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.ymax=log10(g_value_get_double(value));
      break;
    case PROP_XTN:
      GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->ticks.xn=g_value_get_uint(value);
      break;
    case PROP_YTN:
      GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->ticks.yn=g_value_get_uint(value);
      break;
    case PROP_FA:
      GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->flaga=g_value_get_uint(value);
      break;
    default:
...
static void gtk_plot_log_log_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
  switch (prop_id) {
    case PROP_BXN:
      g_value_set_double(value, exp(G_LN10*GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.xmin));
      break;
    case PROP_BXX:
      g_value_set_double(value, exp(G_LN10*GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.xmax));
      break;
    case PROP_BYN:
      g_value_set_double(value, exp(G_LN10*GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.ymin));
      break;
    case PROP_BYX:
      g_value_set_double(value, exp(G_LN10*GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->bounds.ymax));
      break;
    case PROP_XTN:
      g_value_set_uint(value, GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->ticks.xn);
      break;
    case PROP_YTN:
      g_value_set_uint(value, GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->ticks.yn);
      break;
    case PROP_FA:
      g_value_set_uint(value, GTK_PLOT_LOG_LOG_GET_PRIVATE(object)->flaga);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static gboolean gtk_plot_log_log_expose(GtkWidget *widget, GdkEventExpose *event) {
  cairo_t *cr;
...
static void gtk_plot_log_log_class_init(GtkPlotLogLogClass *klass) {
  GObjectClass *obj_klass;
  GtkWidgetClass *widget_klass;

  obj_klass=G_OBJECT_CLASS(klass);
  g_type_class_add_private(obj_klass, sizeof(GtkPlotLogLogPrivate));
  obj_klass->finalize     = (GObjectFinalizeFunc) gtk_plot_log_log_finalise;
  obj_klass->set_property = gtk_plot_log_log_set_property;
  obj_klass->get_property = gtk_plot_log_log_get_property;
  ...
  g_object_class_install_property(obj_klass, PROP_BYN, g_param_spec_double("ymin", "Minimum y value", "Minimum value for the vertical scale", DZE, G_MAXDOUBLE, 0.01, G_PARAM_READWRITE));
  g_object_class_install_property(obj_klass, PROP_BYX, g_param_spec_double("ymax", "Maximum y value", "Maximum value for the vertical scale", DZE, G_MAXDOUBLE, 100.0, G_PARAM_READWRITE));
  ...
  widget_klass->button_press_event   = gtk_plot_log_log_button_press;
  widget_klass->motion_notify_event  = gtk_plot_log_log_motion_notify;
  widget_klass->button_release_event = gtk_plot_log_log_button_release;
  widget_klass->expose_event         = gtk_plot_log_log_expose;
  gtk_plot_log_log_signals[MOVED]       = g_signal_new("moved",       G_OBJECT_CLASS_TYPE(obj_klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(GtkPlotLogLogClass, moved),       NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  gtk_plot_log_log_signals[KEY_CHANGED] = g_signal_new("key_changed", G_OBJECT_CLASS_TYPE(obj_klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(GtkPlotLogLogClass, key_changed), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void gtk_plot_log_log_init(GtkPlotLogLog *plot) {
  GtkPlotLogLogPrivate *priv;
  ...
  plot->flagd=GTK_PLOT_LOG_LOG_DISP_LIN;
  plot->ptsize=5;
  plot->linew=2;
  plot->zmode=GTK_PLOT_LOG_LOG_ZOOM_VRT|GTK_PLOT_LOG_LOG_ZOOM_HZT;
  priv=GTK_PLOT_LOG_LOG_GET_PRIVATE(plot);
  priv->bounds.xmin=priv->bounds.ymin=-2.0;
  priv->bounds.xmax=priv->bounds.ymax=2.0;
  plot->xps=plot->yps=1.0;
  priv->ticks.xn=priv->ticks.yn=8;
  priv->range.xj=priv->range.yj=0;
  priv->range.xn=priv->range.yn=1;
  priv->flaga=priv->flagr=0;
}

GtkWidget *gtk_plot_log_log_new(void) {return g_object_new(GTK_PLOT_TYPE_LOG_LOG, NULL);}
