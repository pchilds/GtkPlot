...
//	xw=alloc.width;
	...
	cairo_rectangle(cr, xw-21.5, 0.5, 10.0, 10.0);
	cairo_rectangle(cr, xw-10.5, 0.5, 10.0, 10.0);
	cairo_move_to(cr, xw-9.0, 5.5);
	cairo_line_to(cr, xw-2.0, 5.5);
	cairo_move_to(cr, xw-5.5, 2.0);
	cairo_line_to(cr, xw-5.5, 9.0);
	if (((plot->zmode)&GTK_PLOT_POLAR_ZOOM_OUT)==0)
	{
		cairo_move_to(cr, xw-6.5, 2.5);
		cairo_line_to(cr, xw-5.5, 2.0);
	...
	if (((plot->zmode)&GTK_PLOT_POLAR_ZOOM_SGL)==0)
	{
		...
		{
			cairo_move_to(cr, xw-19.5, 3.0);
			cairo_line_to(cr, xw-17.5, 9.0);
			cairo_move_to(cr, xw-13.5, 3.0);
			cairo_line_to(cr, xw-15.5, 9.0);
			cairo_stroke(cr);
		}
		if (((plot->zmode)&GTK_PLOT_POLAR_ZOOM_RDL)!=0)
		{
			cairo_move_to(cr, xw-20.0, 3.5);
			cairo_curve_to(cr, xw-16.5, 2.0, xw-16.5, 2.0, xw-13.0, 3.5);
			cairo_move_to(cr, xw-18.0, 8.5);
			cairo_curve_to(cr, xw-16.5, 7.5, xw-16.5, 7.5, xw-15.0, 8.5);
			cairo_stroke(cr);
		}
		cairo_restore(cr);
	}
...
/* drs and drc are the vertical and horizontal distances between rn,tc and rc,tc */
	...
	if (((dt>DZE)||(dt<NZE))&&(((dt+MY_2PI)>DZE)||((dt+MY_2PI)<NZE))&&(((dt-MY_2PI)>DZE)||((dt-MY_2PI)<NZE))) {
		...
		if (dt>=G_PI) {
			...
			if (thx+thn>=0) { /* radial label ccw top to axis azimuthal label bottom to axis */
			...
			} else if (thx+thn>=0) { /* radial label cw top to axis azimuthal label bottom to axis */
			...
			if (dt>=NMY_PI) {
				...
				} else if (thx+thn>=0) { /* radial label ccw bottom to axis azimuthal label top to axis */
					priv->flaga=0;
					sx=dr1+drs;
					if (((dr1>0.0)&&(sx>DZE))||((dr1<0.0)&&(sx<NZE))) {/* check radial y0 to yw */
						sx=((yw/2)-dtt-priv->wr*(1+sin(priv->centre.th)))/sx;
