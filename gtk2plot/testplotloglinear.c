/***************************************************************************
 *            testplotloglinear0-1-0.c
 *
 *  A program to show the capabilities of the plotloglinear widget
 *  version 0.1.0
 *
...
GtkWidget *helpwin, *plot, *sbr, *colour, *jind, *entry1, *entry2, *butt1, *butt2, *jix;

void dpa(GtkWidget *widget, gpointer data)
{
	GArray *car, *cag, *cab, *caa;
	GtkPlot *pt;
	GtkPlotLogLinear *plt;
	GdkColor cl;
	gchar *str, *str2;
	...
	*ptr=iv;
	...
	gtk_plot_log_linear_update_scale(plot, xi, xf, mny, mxy);
}
...
void prt(GtkWidget *widget, gpointer data)
{
	...
	g_signal_connect(prto, "draw_page", G_CALLBACK(gtk_plot_log_linear_print), (gpointer) plot);
...
void prg(GtkWidget *widget, gpointer data)
{
	...
	if (gtk_dialog_run(GTK_DIALOG(wfile))==GTK_RESPONSE_ACCEPT)
	{
		...
		if (filt==epsfilt)
		{
			if (g_str_has_suffix(fout, ".eps")) gtk_plot_log_linear_print_eps(plot, fout);
			else
			{
				fout2=g_strconcat(fout, ".eps", NULL);
				gtk_plot_log_linear_print_eps(plot, fout2);
				g_free(fout2);
			}
		}
		else if (filt==svgfilt)
		{
			if (g_str_has_suffix(fout, ".svg")) gtk_plot_log_linear_print_svg(plot, fout);
			else
			{
				fout2=g_strconcat(fout, ".svg", NULL);
				gtk_plot_log_linear_print_svg(plot, fout2);
				g_free(fout2);
			}
		}
		else if (g_str_has_suffix(fout, ".png")) gtk_plot_log_linear_print_png(plot, fout);
		else
		{
			fout2=g_strconcat(fout, ".png", NULL);
			gtk_plot_log_linear_print_png(plot, fout2);
			g_free(fout2);
		}
		g_free(fout);
	}
...
void opd(GtkWidget *widget, gpointer data)
{
	...
	if (gtk_dialog_run(GTK_DIALOG(wfile))==GTK_RESPONSE_ACCEPT)
	{
		...
		if (g_file_get_contents(fin, &contents, NULL, &Err))
		{
			...
			gtk_plot_log_linear_set_data(plt, x, y, nx, sz, st, ky);
...
void pltmv(GtkPlotLogLinear *plt, gpointer data)
{
	gchar *str;
...
void upg(GtkWidget *widget, gpointer data)
{
	...
	gtk_plot_log_linear_update_scale_pretty(plot, xn, xx, yn, yx);
}
...
int main(int argc, char *argv[])
{
	...
	gchar* klb[3] = {"Quadratic", "Sinusoid", NULL};
	gdouble fll, val;
	...
	gtk_window_set_title(GTK_WINDOW(wdw), "PlotLogLinear tester");
