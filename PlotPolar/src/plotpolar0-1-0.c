 ...
 * BUGS:
 * update pretty out by factor of 10
 * pi doesn't display in pango
...
struct _PlotPolarPrivate {struct xs bounds, rescale; struct pt centre; struct tk ticks; gint x0, y0; gdouble s; guint rcs, thcs, flaga, flagr;};
...
static void draw(GtkWidget *widget, cairo_t *cr)
{
	...
	gdouble dtt, tt, thx, thn, dt, wr, sx, csx, ssx, dr1, drs, drc, dz, rt, dwr, rl, ctx, ctn, stx, stn, r, th, rn, tn, x, y;
	gchar lbl[10];
	gchar* str1=NULL, *str2=NULL, *str3, *pf;
	...
	tt+=ctx;
	tt=sqrt(tt);
	dtt=tt+(plot->lfsize)+JTI;/* determine scale and fit for graph */
	tt=(tt/2)+JTI;
	mtr.xx=1;
	mtr.xy=0;
	mtr.yx=0;
	...
	if (((dt>DZE)||(dt<NZE))&&(((dt+MY_2PI)>DZE)||((dt+MY_2PI)<NZE))&&(((dt-MY_2PI)>DZE)||((dt-MY_2PI)<NZE)))
	{
		...
		if (dt>=G_PI)
		{
			...
			{ /* radial label ccw top to axis azimuthal label bottom to axis */
				...
				{ /* check x1'||x1"=0 to get maximum scale */
					if (thx>=MY_3PI_4) sx=((wr*(cos(priv->centre.th)))-((dtt+wr)*ctx)-(xw/2))/sx;
					else sx=((wr*(cos(priv->centre.th)-ctx))+(dtt*stx)-(xw/2))/sx;
					if (sx<(priv->s)) (priv->s)=sx;
				}
				if (thn<=NMY_PI_2)
				{
				...
				{ /* check y2',y3'=yw to get maximum scale */
					...
					if ((sx>DZE)||(sx<NZE))
					{
						sx=(((wr+dtt)*stn)-(wr*sin(priv->centre.th))+(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
					if ((drs>DZE)||(drs<NZE))
					{
						sx=((wr*(stx-sin(priv->centre.th)))+(dtt*ctx)+(yw/2))/drs;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
			...
			{ /* radial label cw bottom to axis azimuthal label top to axis */
				...
				{ /* check x2'|x2"=0 to get maximum scale */
					if (thn<=N3_MY_PI_4) sx=((wr*cos(priv->centre.th))-((wr+dtt)*ctn)-(xw/2))/sx;
					else sx=((wr*(cos(priv->centre.th)-ctn))-(dtt*stn)-(xw/2))/sx;
					if (sx<(priv->s)) (priv->s)=sx;
				}
				if (thx>=G_PI_2)
				{ /* check y0 to 0 radial to get maximum scale */
				...
				{ /* check y1',y4'=0 to get maximum scale */
					...
					if ((sx>DZE)||(sx<NZE))
					{
						sx=(((wr+dtt)*stx)-(wr*sin(priv->centre.th))-(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
					if ((drs>DZE)||(drs<NZE))
					{
						sx=((wr*(stn-sin(priv->centre.th)))-(dtt*ctn)-(yw/2))/drs;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
		...
		else if (dt>0)
		{
			...
			{ /* radial label ccw top to axis azimuthal label bottom to axis */
				...
				if (((priv->s)>DZE)||((priv->s)<NZE)) (priv->s)=(((wr+dtt)*stx)-(wr*sin(priv->centre.th))+(yw/2))/(priv->s);
				else (priv->s)=G_MAXDOUBLE;
				sx=(dr1*ctx)-drc;
				if ((sx>DZE)||(sx<NZE))
				{/* check x1'||x1"=0 to get maximum scale */
					if (thx>=MY_3PI_4) sx=((wr*(cos(priv->centre.th)))-((wr+dtt)*ctx)-(xw/2))/sx;
					else sx=((wr*(cos(priv->centre.th)-ctx))+(dtt*stx)-(xw/2))/sx;
					if (sx<(priv->s)) (priv->s)=sx;
				}
				if (thn>=G_PI_2)
				{
					...
					{/* check x4=xw to get maximum scale */
						sx=((wr*(ctn-cos(priv->centre.th)))-(xw/2))/drc;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				else
				{
					...
					{/* check x2'=xw to get maximum scale */
						sx=((wr*cos(priv->centre.th))-((wr+dtt)*ctn)+(xw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
			...
			{ /* radial label cw top to axis azimuthal label bottom to axis */
				...
				{/* check y4'=yw to get maximum scale */
					if ((drs>DZE)||(drs<NZE)) (priv->s)=((wr*(stn-(sin(priv->centre.th))))-(dtt*ctn)+(yw/2))/drs;
					else (priv->s)=G_MAXDOUBLE;
					sx=(dr1*ctn)-drc;
					if ((sx>DZE)||(sx<NZE))
					{/* check x2"||x2'=xw to get maximum scale */
						if (thn>=G_PI_4) sx=((wr*cos(priv->centre.th))-((wr+dtt)*ctn)+(xw/2))/sx;
						else sx=((wr*(cos(priv->centre.th)-ctn))-(dtt*stn)+(xw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				...
				{/* check x0 to xw radial to get maximum scale */
					...
					{/* check y2'||y2"=yw to get maximum scale */
						if (thn<=NMY_PI_4) sx=(((wr+dtt)*stn)-(wr*sin(priv->centre.th))+(yw/2))/sx;
						else sx=((wr*(stn-sin(priv->centre.th)))-(dtt*ctn)+(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				if (thx>=G_PI_2)
				{
					...
					{/* check x1'=0 to get maximum scale */
						sx=((wr*(cos(priv->centre.th)))-((dtt+wr)*ctx)-(xw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				else
				{
					...
					{/* check x3=0 to get maximum scale */
						sx=((wr*(ctx-(cos(priv->centre.th))))+(xw/2))/drc;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
			...
			{ /* radial label ccw bottom to axis azimuthal label top to axis */
				...
				{/* check x0 to xw radial to get maximum scale */
					...
					{/* check y1"||y1'=0 to get maximum scale */
						if (thx>=G_PI_4) sx=((wr*(stx-(sin(priv->centre.th))))-(dtt*ctx)-(yw/2))/sx;
						else sx=(((wr+dtt)*stx)-(wr*sin(priv->centre.th))-(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				else
				{/* check y3'=0 to get maximum scale */
					if ((drs>DZE)||(drs<NZE)) (priv->s)=((wr*(stx-sin(priv->centre.th)))+(dtt*ctx)-(yw/2))/drs;
					else (priv->s)=G_MAXDOUBLE;
					sx=(dr1*ctx)-drc;
					if ((sx>DZE)||(sx<NZE))
					{/* check x1"||x1'=xw to get maximum scale */
						if (thx<=NMY_PI_4) sx=((wr*(cos(priv->centre.th)-ctx))+(dtt*stx)-(xw/2))/sx;
						else sx=((wr*(cos(priv->centre.th)))-((wr+dtt)*ctx)+(xw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
				if (thn<=NMY_PI_2)
			...
			{ /* radial label cw bottom to axis azimuthal label top to axis */
				(priv->flaga)=1;
				if ((drs>DZE)||(drs<NZE)) (priv->s)=((wr*(stn-(sin(priv->centre.th))))-(dtt*ctn)-(yw/2))/drs; /* check y4'=0 to get maximum scale */
				else (priv->s)=G_MAXDOUBLE;
				sx=(dr1*ctn)-drc;
				if ((sx>DZE)||(sx<NZE))
				{/* check x2'||x2"=0 to get maximum scale */
				...
				else
				{
					...
					{/* check x1'=xw to get maximum scale */
						sx=((wr*(cos(priv->centre.th)))-((wr+dtt)*ctx)-(xw/2))/sx;;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
				...
				{ /* radial label cw bottom to axis azimuthal label top to axis */
					...
					else
					{
						...
						if (thn>=MY_3PI_4)
						{
							...
							if ((sx>DZE)||(sx<NZE))
							{/* check y2"=0 to get maximum scale */
								sx=((wr*(stn-sin(priv->centre.th)))-(dtt*ctn)-(yw/2))/sx;
								if (sx<(priv->s)) (priv->s)=sx;
							}
						}
					}
					...
					if (thn>=G_PI_2)
					{
						...
						if (thx>=0)
						{
							...
							if (thx<=G_PI_4)
							{
								...
								if ((sx>DZE)||(sx<NZE))
								{/* check y1"=0 */
									sx=((wr*(stx-(sin(priv->centre.th))))-(dtt*ctx)-(yw/2))/sx;
									if (sx<(priv->s)) (priv->s)=sx;
								}
							}
						}
						else
						{
							...
							if ((sx>DZE)||(sx<NZE))
							{/* check x1'||x1"=xw to get maximum scale */
								if (thx>=NMY_PI_4) sx=((wr*(cos(priv->centre.th)))-((dtt+wr)*ctx)+(xw/2))/sx;
								else sx=((wr*(cos(priv->centre.th)-ctx))+(dtt*stx)+(xw/2))/sx;
								if (sx<(priv->s)) (priv->s)=sx;
							}
						}
			...
			{ /* radial label cw bottom to axis azimuthal label top to axis */
				...
				if (thx<=NMY_PI_2)
				{
					...
					else
					{
						...
						if ((drc>DZE)||(drc<NZE))
						{/* check x4'=xw to get maximum scale */
							sx=((wr*(ctn-(cos(priv->centre.th))))+(dtt*stn)-(xw/2))/drc;
							if (sx<(priv->s)) (priv->s)=sx;
						}
					}
				}
				else
				{
					...
					if ((sx>DZE)||(sx<NZE))
					{/* check y2"||y2'=0 */
						if (thn>=MY_3PI_4) sx=((wr*(stn-sin(priv->centre.th)))-(dtt*ctn)-(yw/2))/sx;
						else sx=(((wr+dtt)*stn)-(wr*sin(priv->centre.th))-(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
			}
			...
			{ /* radial label ccw top to axis azimuthal label bottom to axis */
				...
				if (thn>=G_PI_2)
				{
					...
					else
					{
						...
						if ((drc>DZE)||(drc<NZE))
						{/* check x3'=xw to get maximum scale */
							sx=((wr*(ctx-(cos(priv->centre.th))))-(dtt*stx)-(xw/2))/drc;
							if (sx<(priv->s)) (priv->s)=sx;
						}
					}
				}
				else
				{
					...
					if ((sx>DZE)||(sx<NZE))
					{/* check y1"||y1'=yw */
						if (thx<=N3_MY_PI_4) sx=((wr*(stx-(sin(priv->centre.th))))-(dtt*ctx)+(yw/2))/sx;
						else sx=(((wr+dtt)*stx)-(wr*sin(priv->centre.th))+(yw/2))/sx;
						if (sx<(priv->s)) (priv->s)=sx;
					}
				}
		...
		if (((plot->flags)&1)==0)
		{
			for (j=1; j<(priv->ticks.zin); j++)
			{
				for (k=1; k<kx; k++)
				{
					...
					csx=cos(sx);
					ssx=sin(sx);
					cairo_move_to(cr, (priv->x0)+((wr+((priv->s)*dr1))*csx), (priv->y0)-((wr+((priv->s)*dr1))*ssx));
					cairo_line_to(cr, (priv->x0)+((wr+JT+((priv->s)*dr1))*csx), (priv->y0)-((wr+JT+((priv->s)*dr1))*ssx));
				}
				sx+=dz;/* inner radial lines to tick */
				csx=cos(sx);
				ssx=sin(sx);
				cairo_move_to(cr, (priv->x0)+(wr*csx), (priv->y0)-(wr*ssx));
				cairo_line_to(cr, (priv->x0)+((wr+JT+((priv->s)*dr1))*csx), (priv->y0)-((wr+JT+((priv->s)*dr1))*ssx));
				cairo_stroke(cr);
		...
		else
		{
			...
			if ((csx<DZE)||((1-csx)<DZE)) /* check if angles can be rationalised */
			{
				...
				for (j=1; j<(priv->ticks.zin); j++)
				{
					...
					cairo_line_to(cr, (priv->x0)+((wr+JT+((priv->s)*dr1))*csx), (priv->y0)-((wr+JT+((priv->s)*dr1))*ssx));
					cairo_stroke(cr);
					g_ascii_dtostr(str1, (priv->thcs)-jl-1, j+j0);/* convert to fraction in pi */
					str3=g_strconcat(str1, pf, str2, NULL);
					g_free(str1);
		...
		cairo_move_to(cr, (priv->x0)+(wr*ctx), (priv->y0)-(wr*stx));
		cairo_line_to(cr, (priv->x0)+((wr+((priv->s)*dr1))*ctx), (priv->y0)-((wr+((priv->s)*dr1))*stx));
		cairo_stroke(cr);
		...
		if ((priv->flaga)==0)
		{
			...
			for (j=1; j<(priv->ticks.r); j++)
			{
				...
				{/* add additional radials */
					...
					cairo_line_to(cr, (priv->x0)+((wr+((priv->s)*dr1))*csx), (priv->y0)-((wr+((priv->s)*dr1))*ssx));
			...
			pango_layout_get_pixel_size(lyt, &wd, &hg);
			sx=((dr1*(priv->s))-wd)/2;
			cairo_move_to(cr, (priv->x0)+(sx*ctx)-(dtt*stx), (priv->y0)-(dtt*ctx)-(sx*stx));
			cairo_set_matrix(cr, &mtr2);
		...
		else
		{
			...
			pango_layout_get_pixel_size(lyt, &wd, &hg);
			sx=((dr1*(priv->s))+wd)/2;
			cairo_move_to(cr, (priv->x0)+(sx*ctn)+(dtt*stn), (priv->y0)+(dtt*ctn)-(sx*stn));
			cairo_set_matrix(cr, &mtr2);
			pango_cairo_update_layout(cr, lyt);
			pango_cairo_show_layout(cr, lyt);
			g_object_unref(lyt);
	...
	else if ((priv->bounds.rmin)==0)
	{
		...
		else if (fmod((priv->ticks.zin),2)==0) /* change if necessary */
		{
			...
			g_ascii_dtostr(str2, jl, ((priv->ticks.zin)/2));
			...
			for (j=1; j<(priv->ticks.zin); j++)
			{
				...
				cairo_line_to(cr, (priv->x0)+((JT+((priv->s)*dr1))*csx), (priv->y0)-((JT+((priv->s)*dr1))*ssx));
				cairo_stroke(cr);
				g_ascii_dtostr(str1, (priv->thcs)-jl-1, j+j0);
				str3=g_strconcat(str1, pf, str2, NULL);
				g_free(str1);
				lyt=pango_cairo_create_layout(cr); /* draw azimuthal tick labels */
				pango_layout_set_font_description(lyt, dscr2);
		...
		else
		{
			...
			g_ascii_dtostr(str2, jl, (priv->ticks.zin));
		...
		pango_layout_set_text(lyt, (plot->rlab), -1);
		pango_layout_get_pixel_size(lyt, &wd, &hg);
		cairo_move_to(cr, (priv->x0)+(((dr1*(priv->s))-wd)/2), (priv->y0)+dtt-hg);
		pango_cairo_show_layout(cr, lyt);
		g_object_unref(lyt);
		lyt=pango_cairo_create_layout(cr); /* draw azimuthal label */
		pango_layout_set_font_description(lyt, dscr1);
	...
	else
	{
		...
		if (((plot->flags)&1)==0)
		{
			for (j=1; j<(priv->ticks.zin); j++)
			{
				...
				cairo_save(cr);
				cairo_set_dash(cr, &dt, 1, 0);
				cairo_move_to(cr, (priv->x0)+((wr/3)*csx), (priv->y0)-((wr/3)*ssx));
				cairo_line_to(cr, (priv->x0)+(wr*csx), (priv->y0)-(wr*ssx));
				cairo_stroke(cr);
				cairo_restore(cr);
				cairo_line_to(cr, (priv->x0)+((wr+JT+((priv->s)*dr1))*csx), (priv->y0)-((wr+JT+((priv->s)*dr1))*ssx));
				cairo_stroke(cr);
		...
		else if (fmod((priv->ticks.zin),2)==0)
		{
			...
			for (j=1; j<(priv->ticks.zin); j++)
			{
				...
				cairo_save(cr);
				cairo_set_dash(cr, &dt, 1, 0);
				cairo_move_to(cr, (priv->x0)+(wr*csx/3), (priv->y0)-(wr*ssx/3));
				cairo_line_to(cr, (priv->x0)+(wr*csx), (priv->y0)-(wr*ssx));
				cairo_stroke(cr);
				cairo_restore(cr);
				cairo_move_to(cr, (priv->x0)+(wr*csx), (priv->y0)-(wr*ssx));
		...
		pango_layout_set_text(lyt, (plot->rlab), -1);
		pango_layout_get_pixel_size(lyt, &wd, &hg);
		cairo_move_to(cr, (priv->x0)+wr+(((dr1*(priv->s))-wd)/2), (priv->y0)+dtt-hg);
		pango_cairo_show_layout(cr, lyt);
		g_object_unref(lyt);
		lyt=pango_cairo_create_layout(cr); /* draw azimuthal label */
		pango_layout_set_font_description(lyt, dscr1);
	...
	if (plot->rdata && plot->thdata) /* plot data */
	{
		...
		th=g_array_index(plot->thdata, gdouble, 0);
		if (((plot->flags)&2)!=0)
		{
			...
			if (((plot->flags)&4)!=0) /* lines and points */
			{
				if (((plot->flags)&8)==0) /* straight lines */
				{
					...
					{
						xs=0;
						drs=wr+((r-(priv->bounds.rmin))*(priv->s));
					...
					for (j=1; j<(plot->size); j++)
					{
						...
						tn=g_array_index(plot->thdata, gdouble, j);
						if (rn<(priv->bounds.rmin))
						{
							if (tn<(priv->bounds.thmin))
							{
								...
								{
									drs=r-rn;
									if ((drs<DZE)&&(drs>NZE)) {x=(priv->x0)+(wr*cos(priv->bounds.thmin)); y=(priv->y0)-(wr*sin(priv->bounds.thmin));}
									else
									{
										drs=((tn-th)*(r-(priv->bounds.rmin)))/drs;
										drs+=th;
										if (drs>=thn) {x=(priv->x0)+(wr*cos(drs)); y=(priv->y0)-(wr*sin(drs));}/* check wrapping */
										else
										{
											drs=((rn-r)*(th-(priv->bounds.thmin)))/(th-tn);
											drs=wr+((drs+r-(priv->bounds.rmin))*(priv->s));
							...
							else if (tn>(priv->bounds.thmax))
							{
								...
								{
									drs=r-rn;
									if ((drs<DZE)&&(drs>NZE)) {x=(priv->x0)+(wr*cos(priv->bounds.thmax)); y=(priv->y0)-(wr*sin(priv->bounds.thmax));}
									else
									{
										drs=((tn-th)*(r-(priv->bounds.rmin)))/drs;
										drs+=th;
										if (drs<=thx) {x=(priv->x0)+(wr*cos(drs)); y=(priv->y0)-(wr*sin(drs));}
										else
										{
							...
							else
							{
								if (xs==0)
								{
									...
									drs=((tn-th)*(r-(priv->bounds.rmin)))/drs;
									drs+=th;
									x=(priv->x0)+(wr*cos(drs));
									y=(priv->y0)-(wr*sin(drs));
									cairo_line_to(cr, x, y);
									cairo_stroke(cr);
								}
								else if (xs==4) /* check for cutting across */
								{
						...
						else if (tn<(priv->bounds.thmin))
						{
							...
							else if (xs==1)
							{
								...
								if (drs>(priv->bounds.thmin))
								{
									...
									drs=wr+((drs+rn-(priv->bounds.rmin))*(priv->s));
									x=(priv->x0)+(drs*cos(priv->bounds.thmin));
									y=(priv->y0)-(drs*sin(priv->bounds.thmin));
									cairo_line_to(cr, x, y);
									cairo_stroke(cr);
						...
						else /* within range */
						{
							...
							drs=wr+((rn-(priv->bounds.rmin))*(priv->s));
		...
		else if (((plot->flags)&4)!=0) /* points only */
		{
			for (j=0; j<(plot->size); j++)
			{
				...
				if ((r>=(priv->bounds.rmin))&&(r<=(priv->bounds.rmax))&&(th>=(priv->bounds.thmin))&&(th<=(priv->bounds.thmax)))
				{
					drs=wr+((r-(priv->bounds.rmin))*(priv->s));
					x=(priv->x0)+(drs*cos(th));
					y=(priv->y0)-(drs*sin(th));
...
gboolean plot_polar_update_scale_pretty(GtkWidget *widget, gdouble xn, gdouble xx, gdouble yn, gdouble yx)
{
	...
	if (xn<=0)
	{
		...
		else if (num<0.39794) {(priv->ticks.r)=ceil(num*20); (priv->bounds.rmax)=5*(priv->ticks.r)*exp(G_LN10*(--num2));}
		else if (num<0.69897) {(priv->ticks.r)=ceil(num*10); (priv->bounds.rmax)=(priv->ticks.r)*exp(G_LN10*num2);}
		else {(priv->ticks.r)=ceil(num*5); (priv->bounds.rmax)=2*(priv->ticks.r)*exp(G_LN10*num2);}
...
static void plot_polar_init(PlotPolar *plot)
{
	...
	priv=PLOT_POLAR_GET_PRIVATE(plot);
	{(priv->bounds.rmin)=0; (priv->bounds.rmax)=1; (priv->bounds.thmin)=-1.5; (priv->bounds.thmax)=2;}
	{(priv->centre.r)=0.5; (priv->centre.th)=0;}
	{(priv->ticks.r)=4; (priv->ticks.zin)=12; (priv->ticks.z2m)=2; (priv->ticks.zc)=50.0;}
	{(priv->rcs)=5; (priv->thcs)=5;}
