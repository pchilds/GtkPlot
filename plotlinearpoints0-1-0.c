...
#include <math.h>
#include <cairo-ps.h>
#include "plotlinearpoints0-1-0.h"

#define PLOT_LINEAR_POINTS_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), PLOT_TYPE_LINEAR_POINTS, PlotLinearPointsPrivate))
...
G_DEFINE_TYPE (PlotLinearPoints, plot_linear_points, GTK_TYPE_DRAWING_AREA);
enum
  {
  PROP_0,
  PROP_BXN,
...
static guint plot_linear_points_signals[LAST_SIGNAL]={0};
typedef struct _PlotLinearPointsPrivate PlotLinearPointsPrivate;
...
struct _PlotLinearPointsPrivate
  {
  struct xs bounds;
  struct tk ticks;
  guint xcs;
...
static void plot_linear_points_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
  {
  PlotLinearPointsPrivate *priv;

  priv=PLOT_LINEAR_POINTS_GET_PRIVATE(object);
  switch (prop_id)
    {
    case PROP_BXN:
...
static void plot_linear_points_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
  {
  PlotLinearPointsPrivate *priv;
...
static void plot_linear_points_finalise(PlotLinearPoints *plot)
  {
  PlotLinearPointsPrivate *priv;

  priv=PLOT_LINEAR_POINTS_GET_PRIVATE(plot);
...
static void draw_points(GtkWidget *widget, cairo_t *cr)
  {
  PlotLinearPointsPrivate *priv;
  PlotLinearPoints *plot;
  gint j, k, xw, yw, xr, xr2, yr, yr2, xa, ya, xl, yl, xu, yu, tf, tz, to, tn, tnn, xv, yv, dtt;
  gdouble dt;
  gchar lbl[10];
  ...
  mtr3.yx=1;
  mtr3.yy=0;
  plot=PLOT_LINEAR_POINTS(widget);
...
static void plot_linear_points_redraw(GtkWidget *widget)
  {
  GdkRegion *region;
...
gboolean plot_linear_points_update_scale(GtkWidget *widget, gdouble xn, gdouble xx, gdouble yn, gdouble yx)
  ...
  priv=PLOT_LINEAR_POINTS_GET_PRIVATE(widget);
  priv->bounds.xmin=xn;
  priv->bounds.xmax=xx;
  priv->bounds.ymin=yn;
  priv->bounds.ymax=yx;
  plot_linear_points_redraw(widget);
  return FALSE;
  }
...
gboolean plot_linear_points_print_eps(GtkWidget *plot, gchar* fout)
  {
  cairo_t *cr;
...
static gboolean plot_linear_points_button_press(GtkWidget *widget, GdkEventButton *event)
  {
  PlotLinearPointsPrivate *priv;
  PlotLinearPoints *plot;
  gint d;
...
static gboolean plot_linear_points_motion_notify(GtkWidget *widget, GdkEventMotion *event)
  {
  PlotLinearPointsPrivate *priv;
  PlotLinearPoints *plot;
  gdouble dx, dy;
  ...
  if ((dx>=0)&&(dy>=0)&&(dx<=1)&&(dy<=1))
    ...
    g_signal_emit(plot, plot_linear_points_signals[MOVED], 0);
    }
  return FALSE;
  }

static gboolean plot_linear_points_button_release(GtkWidget *widget, GdkEventButton *event)
  {
  PlotLinearPointsPrivate *priv;
  PlotLinearPoints *plot;
  gint d, xw;
  ...
  if ((priv->flagr)==1)
      ...
      if (((plot->zmode)&1)==0)
        ...
        if ((priv->rescale.xmax)>(priv->rescale.xmin))
          ...
          if ((priv->rescale.ymax)>(priv->rescale.ymin))
            ...
            plot_linear_points_update_scale_pretty(widget, xn, xx, yn, yx);
            }
    ...
    if ((event->x)>=xw-20)
      {
      if ((event->x)>xw-10) {(plot->zmode)^=1; plot_linear_points_redraw(widget);}
      else if (((plot->zmode)&8)!=0) {(plot->zmode)&=1; plot_linear_points_redraw(widget);}
      else {(plot->zmode)++; (plot->zmode)++; plot_linear_points_redraw(widget);}
      }
    }
  return FALSE;
  }

static gboolean plot_linear_points_expose(GtkWidget *plot, GdkEventExpose *event)
  {
  cairo_t *cr;
...
static void plot_linear_points_class_init(PlotLinearPointsClass *klass)
  {
  GObjectClass *obj_klass;
  GtkWidgetClass *widget_klass;

  obj_klass=G_OBJECT_CLASS(klass);
  g_type_class_add_private(obj_klass, sizeof(PlotLinearPointsPrivate));
  obj_klass->finalize=(GObjectFinalizeFunc) plot_linear_points_finalise;
  obj_klass->set_property=plot_linear_points_set_property;
  obj_klass->get_property=plot_linear_points_get_property;
  ...
  widget_klass->button_press_event=plot_linear_points_button_press;
  widget_klass->motion_notify_event=plot_linear_points_motion_notify;
  widget_klass->button_release_event=plot_linear_points_button_release;
  widget_klass->expose_event=plot_linear_points_expose;
  plot_linear_points_signals[MOVED]=g_signal_new("moved", G_OBJECT_CLASS_TYPE(obj_klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (PlotLinearPointsClass, moved), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  }

static void plot_linear_points_init(PlotLinearPoints *plot)
  {
  PlotLinearPointsPrivate *priv;
...
GtkWidget *plot_linear_points_new(void)
  {
  return g_object_new(PLOT_TYPE_LINEAR_POINTS, NULL);
  }
