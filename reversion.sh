...
then
	...
	echo "	gtkplotloglog.c"
	echo "gtkplotinclude_HEADERS+=					\\"
	echo "	gtkplotloglinear.h						\\"
	echo "	gtkplotloglog.h"
	echo "endif"
	echo "if PL3D"
	echo "libgtkplot_@PLOT_API_VERSION@_la_SOURCES+=	\\"
	echo "	gtksurf3d.c"
	...
	echo "bin_PROGRAMS="
	echo "if BASE"
	echo "bin_PROGRAMS+=testPlotLinear"
	echo "testPlotLinear_SOURCES=testplotlinear.c"
	echo "testPlotLinear_LDADD=\$(PLOT_LIBS) -lm libgtkplot-@PLOT_API_VERSION@.la"
	echo "endif"
	echo "if POLARS"
	echo "bin_PROGRAMS+=testPlotPolar"
	echo "testPlotPolar_SOURCES=testplotpolar.c"
	echo "testPlotPolar_LDADD=\$(PLOT_LIBS) -lm libgtkplot-@PLOT_API_VERSION@.la"
	echo "endif"
	echo "if LOGARS"
	echo "bin_PROGRAMS+=testPlotLogLinear"
	echo "bin_PROGRAMS+=testPlotLogLog"
	...
	echo "testPlotLogLog_SOURCES=testplotloglog.c"
	echo "testPlotLogLog_LDADD=\$(PLOT_LIBS) -lm libgtkplot-@PLOT_API_VERSION@.la"
	echo "endif"
	echo "if MISCS"
	echo "bin_PROGRAMS+=testPlotDonut"
...
echo "AC_ARG_ENABLE([base],AS_HELP_STRING([--disable-base],[Do not include the base plotlinear widget  @<:@yes/no@:>@]),,[enable_base=yes])"
echo "AM_CONDITIONAL([BASE], [test \"\$enable_base\" = \"yes\"])"
echo "AC_ARG_ENABLE([polars],AS_HELP_STRING([--disable-polar],[Do not include polar plots  @<:@yes/no@:>@]),,[enable_polars=yes])"
echo "AM_CONDITIONAL([POLARS], [test \"\$enable_polars\" = \"yes\"])"
echo "AC_ARG_ENABLE([logarithms],AS_HELP_STRING([--disable-logarithms],[Do not include logarithmic plots  @<:@yes/no@:>@]),,[enable_logarithms=yes])"
echo "AM_CONDITIONAL([LOGARS], [test \"\$enable_logarithms\" = \"yes\"])"
echo "AC_ARG_ENABLE([plot_3d],AS_HELP_STRING([--disable-plot-3d],[Do not include 3D plots  @<:@yes/no@:>@]),,[enable_plot_3d=yes])"
echo "AM_CONDITIONAL([PL3D], [test \"\$enable_plot_3d\" = \"yes\"])"
echo "AC_ARG_ENABLE([miscs],AS_HELP_STRING([--disable-miscs],[Do not include other miscelaneous plots  @<:@yes/no@:>@]),,[enable_miscs=yes])"
echo "AM_CONDITIONAL([MISCS], [test \"\$enable_miscs\" = \"yes\"])"
echo "AS_IF([test \"\$enable_plot_3d\" = \"yes\"],[PKG_CHECK_MODULES([PLOT], [gtk+-${gmx}.${gar} >= ${gmx}.${gmn} gtkglext-1.0])],[PKG_CHECK_MODULES([PLOT], [gtk+-${gmx}.${gar} >= ${gmx}.${gmn}])])"
