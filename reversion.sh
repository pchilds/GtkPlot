...
echo "Release:			1%{?dist}"
echo "Licence:			LGPLv${gmx}+"
echo "Group:			Development/Libraries"
echo "Source:			%{name}-%{version}.tar.gz"
echo "BuildRequires:	automake, autoconf, libtool pkg-config(gtk+-${gmx}.${gar} >= ${gmx}.${gmn})"
echo "Requires:			gtk+${gmx} >= ${gmx}.${gar}"
echo ""
echo "%description"
echo "%{name}-${gmx} makes available to gtk+${gmx} programs, a pi/donut, "
...
echo ""
echo "%package devel"
echo "Summary:			Provides headers for the %{name}-${gmx} package"
echo "Group:			Development/Libraries"
echo "Requires:			%{name} = %{version}-%{release}, pkgconfig"
echo ""
echo "%description devel"
echo "This package contains the header files and libraries needed to develop "
echo "applications that use %{name}-${gmx}."
echo ""
echo "%package static"
echo "Summary:			Static development files for %{name}-${gmx}"
echo "Group:			Development/Libraries"
echo "Requires:			%{name} = %{version}-%{release}"
echo ""
echo "%description static"
echo "This package contains static libraries to develop applications that use %{name}-${gmx}."
echo ""
echo "%prep"
echo "%autosetup -n %{name}"
echo ""
echo "%build"
...
echo "%{_includedir}/*"
echo "/%{_lib}/*.so"
echo "/%{_lib}/pkgconfig/%{name}-${gmx}.${gar}.pc"
echo ""
echo "%files static"
echo "%defattr(-,root,root)"
echo "/%{_lib}/*.a"
...
echo "- Initial build"
} >SPECS/gtkplot-${gmx}.${lcr}.${lrv}.spec
{
echo "AC_PREREQ([2.63])"
